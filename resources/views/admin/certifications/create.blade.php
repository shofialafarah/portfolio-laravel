...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('admin.certifications.index') }}" class="text-indigo-400 hover:text-indigo-300 text-sm font-medium flex items-center gap-2 mb-2">
    ...
    <div class="relative group">
        ...
        <form action="{{ route('admin.certifications.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                <label class="block text-sm font-bold text-zinc-400 uppercase tracking-widest mb-2">Judul Sertifikat <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: AWS Certified Solutions Architect" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-bold text-zinc-400 uppercase tracking-widest mb-2">Penyelenggara <span class="text-red-500">*</span></label>
                    <input type="text" name="issuer" value="{{ old('issuer') }}" placeholder="Contoh: Udemy, Google, Dicoding" required
                ...
                <div>
                    <label class="block text-sm font-bold text-zinc-400 uppercase tracking-widest mb-2">Tahun <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-bold text-zinc-400 uppercase tracking-widest mb-2">Upload Sertifikat <span class="text-red-500">*</span></label>
                <input type="file" name="image" id="cert-image" accept="image/*" required class="hidden">
                <label for="cert-image" class="w-full bg-zinc-900/50 border-2 border-dashed border-white/10 text-zinc-500 rounded-xl px-4 py-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all">
            ...
            <label class="flex items-center gap-3 p-4 bg-zinc-900/30 border border-white/5 rounded-2xl cursor-pointer hover:bg-zinc-900/50 transition-all">
                <input type="checkbox" name="is_active" value="1" checked class="w-5 h-5 rounded border-zinc-700 bg-zinc-800 text-emerald-500 focus:ring-emerald-500">
...
<script>
    document.getElementById('cert-image').addEventListener('change', function(e) {
        document.getElementById('file-name').textContent = e.target.files[0] ? e.target.files[0].name : "Klik untuk upload JPG/PNG";
    });
</script>
@endsection