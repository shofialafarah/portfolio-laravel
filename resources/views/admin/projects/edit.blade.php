...
<div class="max-w-3xl mx-auto" x-data="{ category: '{{ $project->category }}' }">
    <div class="mb-8 text-center md:text-left">
        <a href="{{ route('admin.projects.index', ['category' => $project->category]) }}" class="text-indigo-400 hover:text-indigo-300 text-sm font-medium inline-flex items-center gap-2 mb-2 transition-colors">
    ...
    <div class="relative group">
        ...
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="space-y-6 pt-6 border-t border-white/5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="url" name="link_deploy" value="{{ old('link_deploy', $project->link_deploy) }}" placeholder="https://..." 
                            class="w-full bg-zinc-900/50 border border-white/10 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    </div>
                    ...
                    <div x-show="category === 'web'" x-transition x-cloak>
                        ...
                        <input type="url" name="link_github" value="{{ old('link_github', $project->link_github) }}" placeholder="https://github.com/..." 
                            class="w-full bg-zinc-900/50 border border-white/10 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    </div>
                </div>
                ...
                <div x-show="category === 'web'" x-transition x-cloak>
                    ...
                    <input type="text" name="tech_stack" value="{{ old('tech_stack', $project->tech_stack) }}" placeholder="Contoh: Laravel, Tailwind, MySQL" 
                        class="w-full bg-zinc-900/50 border border-white/10 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-4 pt-4">
                <button type="submit" class="flex-1 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-xl transition-all shadow-lg shadow-indigo-500/20 uppercase tracking-wider active:scale-95">
                    Update Projek
                </button>
                <a href="{{ route('admin.projects.index', ['category' => $project->category]) }}" class="px-8 py-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-bold rounded-xl transition-all text-center">
                    Batal
                </a>
            </div>
        </form>
